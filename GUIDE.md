...
```
[원본 마크다운 문서] -> [문서 처리/청킹] -> [임베딩 생성] -> [ChromaDB 저장]
```

#### 쿼리 처리 흐름

```
사용자 질문 -> 웹 UI -> FastAPI -> 벡터 검색 -> 관련 문서 검색 -> GPT 응답 생성 -> 사용자에게 반환
```
...
```
마크다운 문서 -> 청킹(splitting) -> 벡터화(embedding) -> ChromaDB 저장
```

##### 사용자 쿼리 처리 단계:

```
사용자 질문 -> 질문 벡터화 -> 유사 문서 검색 -> 컨텍스트 구성 -> 프롬프트 생성 -> GPT 응답 생성
```

##### 웹 인터페이스 흐름:

```
사용자 입력 -> JavaScript 처리 -> API 요청 -> 응답 처리 -> UI 업데이트
```

##### 데이터 흐름:

```
문서(docs/) -> 처리(markdown_processor) -> 벡터(embeddings) -> 저장소(ChromaDB) -> 검색(rag) -> 응답(API)
```

이 다이어그램들은 사용자 입력부터 시작하여 웹 인터페이스, 백엔드 처리, 벡터 검색, 응답 생성까지의 전체 흐름을 보여줍니다.
