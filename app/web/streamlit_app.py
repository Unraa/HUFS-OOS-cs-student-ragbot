...
# 페이지 설정
st.set_page_config(
    page_title="한국외국어대학교 컴퓨터공학과 RAG 챗봇",
    page_icon="💬",
    layout="centered",
)
...
# 제목 및 소개
st.title("한국외국어대학교 컴퓨터공학과 RAG 챗봇")
...
if not st.session_state.get("prompts_loaded", False):
    status_messages.append(
        "⚠️ 프롬프트 로드 실패: 'app/core/prompts.yaml' 파일 확인 필요"
    )
else:
...
if not st.session_state.get("vector_store_loaded", False):
    status_messages.append(
        "⚠️ 벡터 저장소 로드 실패: 'data/vector_store.json' 파일 확인 필요"
    )
else:
    status_messages.append(
...
with st.sidebar:
    ...
    st.markdown(
        """
    이 챗봇은 한국외국어대학교 컴퓨터공학과 관련 문서를 기반으로 RAG(Retrieval-Augmented Generation) 기술을 활용하여 
    질문에 답변합니다.
